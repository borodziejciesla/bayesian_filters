...
namespace bf
{
    ...
    void ExtendedKalmanFilter::Correction(const bf_io::ValueWithTimestampAndCovariance & measurement) {
        auto [y, R] = ConvertMeasurement(measurement);
        Eigen::MatrixXf observation_jacobian = observation_jacobian_(predicted_state_);

        Eigen::MatrixXf predicted_measurement = observation_(predicted_state_);
        ...
        estimated_covariance_ = static_cast<Eigen::MatrixXf>(tmp * predicted_covariance_ * tmp.transpose())
